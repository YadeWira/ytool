...
var
  ...
  { Estado compartido con el trabajador ReplaceThread (antes capturado por un
...
procedure ReplaceThread(X: IntPtr);
var
  Ptr: PByte;
  Y: Integer;
  C: Word;
            ...
            for J := Low(Tasks) to High(Tasks) do
              if Tasks[J].Status = TThreadStatus.tsErrored then
                try
                  ...
                  on E: Exception do
                    WriteLine(Format('replace: worker #%d failed: %s', [J, E.Message]));
                end;
            for J := Low(Tasks) to High(Tasks) do
            begin
              InfoStore[J].Index := 0;
